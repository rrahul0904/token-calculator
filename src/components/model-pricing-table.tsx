...
export function ModelPricingTable() {
  ...
  return <tr key={model.id}><td><strong><Link href={`/models/${model.id}`}>{model.name}</Link></strong><span>{model.provider}{model.status ? ` · ${model.status}` : ""}</span></td><td>{formatTokens(model.contextWindow)}{model.maxOutput ? <span>{formatTokens(model.maxOutput)} max output</span> : null}</td><td>${resolved.pricing.input.toFixed(resolved.pricing.input < 1 ? 3 : 2)}</td><td>{resolved.pricing.cachedInput === undefined ? "Not offered" : `${resolved.pricing.cachedInput.toFixed(resolved.pricing.cachedInput < 1 ? 3 : 2)}`}</td><td>${resolved.pricing.output.toFixed(resolved.pricing.output < 1 ? 3 : 2)}</td><td>{model.longContext ? <span className="tier-pill">Auto tier above {formatTokens(model.longContext.threshold)}</span> : <span className="muted">{resolved.tier}</span>}</td><td><a href={resolved.sourceUrl} target="_blank" rel="noreferrer">Official ↗</a><span>{resolved.verifiedAt}</span></td></tr>;
