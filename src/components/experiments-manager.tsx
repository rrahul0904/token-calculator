...
export function ExperimentsManager({ canManage }: { canManage: boolean }) {
  ...
  async function addCase() {
    ...
    await execute(async () => {
      ...
      setCaseReference(""); setMessage("Evaluation case added without storing prompt content."); await load(); router.refresh();
    });
  }
  ...
  async function recordResult() {
    ...
    await execute(async () => {
      ...
      setMessage(`${variant === "baseline" ? "Baseline" : "Candidate"} observation recorded.`); setGate(null); await load(); router.refresh();
    });
  }
  ...
  return <section className="app-panel">
    <div className="app-panel__header"><div><h2>Experiment workbench</h2><p>Create versioned metadata-only evaluation datasets, record controlled baseline/candidate evidence, and run a deterministic savings gate.</p></div></div>
    <div className="app-panel__body app-stack">
      {!canManage ? <p>You can inspect experiment evidence, but your organization role cannot create or modify experiments.</p> : <>
        <div className="app-grid">
          <section className="app-panel"><div className="app-panel__header"><div><h3>1. Dataset</h3><p>References only; prompt/code content is not stored.</p></div></div><div className="app-panel__body app-stack">
          ...
          <section className="app-panel"><div className="app-panel__header"><div><h3>2. Experiment</h3><p>Compare the same evaluation workload under two controlled configurations.</p></div></div><div className="app-panel__body app-stack">
        ...
        <section className="app-panel"><div className="app-panel__header"><div><h3>3. Evidence</h3><p>Verification requires at least five baseline and five candidate observations, completed status, non-inferior success/quality, and lower median cost.</p></div></div><div className="app-panel__body app-stack">
          ...
          <div className="form-actions"><button className="button button--ghost" type="button" disabled={busy || !experimentId} onClick={() => void recordResult()}>Record observation</button><button className="button button--primary" type="button" disabled={busy || !experimentId} onClick={() => void completeAndEvaluate()}>Complete + evaluate gate</button></div>
          {gate ? <div className="finding"><div className="finding__top"><div><strong>{gate.passed ? "Verified savings" : "Not verified"}</strong><p>{gate.evidenceType} · baseline n={gate.baseline?.sampleSize ?? 0} · candidate n={gate.candidate?.sampleSize ?? 0}</p></div></div></div> : null}
        </section>
      </>}
