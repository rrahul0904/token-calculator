...
export const experimentResults = pgTable("experiment_results", {
  ...
  economicsSource: text("economics_source").notNull().default("unknown"),
  measurementScope: text("measurement_scope").notNull().default("unknown"),
  benchmarkContext: jsonb("benchmark_context").$type<Record<string, unknown>>().notNull().default({}),
