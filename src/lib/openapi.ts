...
export const OPENAPI_DOCUMENT = {
  ...
  info: {
    ...
    version: "1.3.0",
    description: "AI economics, model discovery, workload planning, agent-run telemetry, FinOps, budgets, governed provider gateway and MCP APIs. Measurement provenance is preserved as provider_measured, agent_measured, local_tokenizer_reference, estimated, or reconciled. Prompt/code content is not persisted by default.",
  },
  ...
  components: {
    ...
    schemas: {
      ...
      TokenizeRequest: {
        type: "object",
        properties: {
          text: { type: "string", maxLength: 500000, description: "Text is processed for the response only; the UTF-8 request text must not exceed 500,000 bytes." },
          model: { type: "string", description: "Optional Token Intelligence model ID used to select a tokenizer family." },
          includePieces: { type: "boolean", default: false },
          maxPieces: { type: "integer", minimum: 1, maximum: 500, default: 100 },
        },
        required: ["text"],
      },
      ...
      WorkloadScenario: {
        type: "object",
        properties: {
          mode: { type: "string", enum: ["tokens2cost", "cost2tokens"] },
          modelId: { type: "string" },
          ...
          cacheHitPercent: { type: "number", minimum: 0, maximum: 100 },
          cacheableInputPercent: { type: "number", minimum: 0, maximum: 100 },
          cacheWrite5mPercent: { type: "number", minimum: 0, maximum: 100 },
          cacheWrite1hPercent: { type: "number", minimum: 0, maximum: 100 },
          requestsPerMonth: { type: "integer", minimum: 0 },
        },
  ...
  paths: {
    ...
    "/api/v1/models/{id}/endpoints": { get: operation("List inference endpoints for a canonical model", { tags: ["Economics"] }) },
    "/api/v1/pricing": { get: operation("List effective endpoint pricing and catalog publication provenance", { tags: ["Economics"] }) },
    ...
    "/api/v1/economics/estimate": { post: operation("Estimate a shareable workload scenario", { request: { $ref: "#/components/schemas/WorkloadScenario" }, tags: ["Economics"] }) },
    "/api/v1/economics/reverse": { post: operation("Solve workload token capacity for a budget", { request: { $ref: "#/components/schemas/WorkloadScenario" }, tags: ["Economics"] }) },
    "/api/v1/economics/compare": { post: operation("Compare a pinned workload baseline across models", { tags: ["Economics"] }) },
    "/api/v1/economics/frontier": { post: operation("Compute an evidence-backed cost/quality Pareto frontier", { tags: ["Economics"] }) },
    ...
    "/api/v1/scenarios/{id}/variance": { get: operation("Reconcile a saved workload scenario with an agent run", { security: session, tags: ["Economics", "Runs"] }) },
