...
export const OPENAPI_DOCUMENT = {
    ...
    schemas: {
      ...
      BudgetCheckRequest: {
        ...
        properties: {
          ...
          runId: { type: "string" },
          observedCostUsd: { type: "number", minimum: 0, default: 0 },
          projectedNextCallCostUsd: { type: "number", minimum: 0 },
          tokens: { type: "integer", minimum: 0, default: 0 },
          turns: { type: "integer", minimum: 0, default: 0 },
          retries: { type: "integer", minimum: 0, default: 0 },
          failedToolCalls: { type: "integer", minimum: 0, default: 0 },
          toolCalls: { type: "integer", minimum: 0, default: 0 },
          elapsedMs: { type: "integer", minimum: 0, default: 0, description: "Observed wall-clock runtime in milliseconds." },
          providerRounds: { type: "integer", minimum: 0, default: 0, description: "Observed provider request rounds, including retries/fallback rounds when the caller counts them." },
          resultBytes: { type: "integer", minimum: 0, default: 0, description: "Observed result payload size in bytes." },
          actionRisk: { type: "string", enum: ["low", "medium", "high", "critical"], description: "Explicit action risk supplied by the caller; Token Intelligence does not infer private task risk from prompt content." },
          actionCategory: { type: "string", maxLength: 120 },
          actionName: { type: "string", maxLength: 240 },
          contextUtilizationPct: { type: "number", minimum: 0, maximum: 100 },
          provider: { type: "string" },
          model: { type: "string" },
          fallbackPremiumUsd: { type: "number", minimum: 0 },
  ...
  paths: {
    ...
    "/api/v1/outcomes/economics": { get: operation("Summarize high-confidence run-to-outcome economics", { security: bearer, tags: ["Economics"], description: "Returns outcome-linked AI cost using existing run and outcome receipts. Actual/reconciled cost is kept separate from estimated-only spend, low-confidence associations are excluded from derived outcome unit economics, and stable CI/deployment identities enable unique shipped-outcome accounting when provided." }) },
    "/api/v1/experiments/{id}/savings-verifications": { get: operation("List verified savings snapshots", { security: session, tags: ["Experiments"] }), post: operation("Record qualifying verified savings evidence", { security: session, tags: ["Experiments"] }) },
    "/api/v1/experiments/{id}/savings-revalidations": { get: operation("List verified savings revalidation checks", { security: session, tags: ["Experiments"] }), post: operation("Revalidate the latest verified savings evidence", { security: session, tags: ["Experiments"], description: "Re-evaluates current experiment evidence without mutating prior savings receipts. Changed qualifying evidence may create a new verified-savings version; failures are recorded as append-only revalidation evidence." }) },
    ...
    "/api/v1/budgets/check": { post: operation("Evaluate hierarchical policy/budget state", { security: bearer, request: { $ref: "#/components/schemas/BudgetCheckRequest" }, tags: ["Control"], description: "Evaluates caller-observed spend, tokens, turns, retries, tools, elapsed wall-clock time, provider rounds, result bytes and model/provider state against restrictive hierarchical policies." }) },
