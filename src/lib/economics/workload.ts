...
export function estimateWorkload(model: ModelCatalogEntry, scenario: WorkloadScenario): WorkloadEstimate {
  ...
  const tier = routedEndpoint
    ...
    : endpoint
      ? `${endpoint.inferenceProvider} · ${modelTier.tier}`
      : modelTier.tier;
  ...
  const cacheWrite5mUsd = componentCost(
    buckets.cacheWrite5mTokens,
    pricing.cacheWrite5m ?? routedEndpoint?.pricing.cacheWrite,
  );
  const cacheWrite1hUsd = componentCost(
    buckets.cacheWrite1hTokens,
    pricing.cacheWrite1h ?? routedEndpoint?.pricing.cacheWrite,
  );
  const outputUsd = componentCost(buckets.outputTokens, pricing.output);
  const totalUsd = sumKnown([
    freshInputUsd,
    cachedReadUsd,
    cacheWrite5mUsd,
    cacheWrite1hUsd,
  ...
  const cacheSavingsUsd = totalUsd === null || noCacheCostUsd === null
    ? null
    : noCacheCostUsd - totalUsd;
  const cacheSavingsPercent = cacheSavingsUsd === null || noCacheCostUsd === null || noCacheCostUsd === 0
    ? null
    : cacheSavingsUsd / noCacheCostUsd * 100;
  const monthlyCostUsd = totalUsd === null
    ? null
    : totalUsd * Math.max(0, finite(scenario.requestsPerMonth));
  ...
  return {
    ...
    contextUtilizationPercent: contextWindow && contextWindow > 0
      ? Math.min(999, contextTotal / contextWindow * 100)
      : 0,
  };
}

export function solveTokensForBudget(
  model: ModelCatalogEntry,
  scenario: WorkloadScenario,
): WorkloadEstimate | null {
  const budget = Math.max(0, finite(scenario.budgetUsd));
  if (budget === 0) {
    return estimateWorkload(model, { ...scenario, totalTokens: 0, mode: "tokens2cost" });
  }

  const probe = estimateWorkload(model, {
    ...scenario,
  ...
  while (high < MAX_PLANNING_TOKENS) {
    const estimate = estimateWorkload(model, {
      ...scenario,
      totalTokens: high,
      mode: "tokens2cost",
    });
...
export function resolveScenarioEstimate(scenario: WorkloadScenario): WorkloadEstimate | null {
  ...
  return scenario.mode === "cost2tokens"
    ? solveTokensForBudget(model, scenario)
    : estimateWorkload(model, scenario);
}

export function compareWithPinned(
  baselineModel: ModelCatalogEntry,
  candidateModel: ModelCatalogEntry,
  scenario: WorkloadScenario,
): PinnedComparison {
  const baseline = estimateWorkload(baselineModel, {
    ...scenario,
    modelId: baselineModel.id,
    endpointId: baselineModel.id === scenario.modelId ? scenario.endpointId : null,
  });
  const candidate = estimateWorkload(candidateModel, {
    ...scenario,
    modelId: candidateModel.id,
    endpointId: candidateModel.id === scenario.modelId ? scenario.endpointId : null,
  });
  const baseCost = baseline.cost.totalUsd;
  const candidateCost = candidate.cost.totalUsd;
  const requestCostDeltaUsd = baseCost === null || candidateCost === null
    ? null
    : candidateCost - baseCost;
  const requestCostDeltaPercent = requestCostDeltaUsd === null || baseCost === null || baseCost === 0
    ? null
    : requestCostDeltaUsd / baseCost * 100;
  const monthlyCostDeltaUsd = baseline.monthlyCostUsd === null || candidate.monthlyCostUsd === null
    ? null
    : candidate.monthlyCostUsd - baseline.monthlyCostUsd;
  ...
  return {
    ...
    monthlyCostDeltaUsd,
    classification:
      baselineModel.id === candidateModel.id
        ? "same_model"
        : baselineModel.provider === candidateModel.provider
          ? "same_provider"
          : "economic_alternative",
    qualityEquivalent: false,
  };
}
...
export function computeCostQualityFrontier(candidates: FrontierCandidate[]): FrontierResult {
  ...
  const frontier = evidenced.filter((candidate) => !evidenced.some((other) => {
    ...
    const noWorse =
      (other.costUsd as number) <= (candidate.costUsd as number) &&
      (other.qualityScore as number) >= (candidate.qualityScore as number);
  ...
  return {
    ...
    frontier,
    omittedWithoutEvidence: candidates
      .filter((candidate) => !evidenced.includes(candidate))
...
export function parseWorkloadQuery(input: string | URLSearchParams): WorkloadScenario {
  ...
  const modelId = MODEL_CATALOG.some((model) => model.id === requestedModel)
    ? requestedModel
    : DEFAULT_WORKLOAD_SCENARIO.modelId;
  ...
  return {
    ...
    pinnedModelId: requestedPin && MODEL_CATALOG.some((model) => model.id === requestedPin)
      ? requestedPin
      : null,
    totalTokens: clampPlanningTokens(
      queryNumber(params, "tokens", DEFAULT_WORKLOAD_SCENARIO.totalTokens),
    ),
    budgetUsd: Math.min(
      MAX_PLANNING_BUDGET_USD,
      Math.max(0, queryNumber(params, "budget", DEFAULT_WORKLOAD_SCENARIO.budgetUsd)),
    ),
    inputPercent: clampPercent(
      queryNumber(params, "input", DEFAULT_WORKLOAD_SCENARIO.inputPercent),
    ),
    cacheHitPercent: clampPercent(
      queryNumber(params, "cache", DEFAULT_WORKLOAD_SCENARIO.cacheHitPercent),
    ),
    cacheableInputPercent: clampPercent(
      queryNumber(params, "cacheable", DEFAULT_WORKLOAD_SCENARIO.cacheableInputPercent),
    ),
    cacheWrite5mPercent: clampPercent(
      queryNumber(params, "write5m", DEFAULT_WORKLOAD_SCENARIO.cacheWrite5mPercent),
    ),
    cacheWrite1hPercent: clampPercent(
      queryNumber(params, "write1h", DEFAULT_WORKLOAD_SCENARIO.cacheWrite1hPercent),
    ),
    requestsPerMonth: Math.min(
      MAX_REQUESTS_PER_MONTH,
      Math.max(
        0,
        Math.round(queryNumber(params, "requests", DEFAULT_WORKLOAD_SCENARIO.requestsPerMonth)),
      ),
    ),
  };
}
...
export function serializeWorkloadQuery(scenario: WorkloadScenario) {
  ...
  if (scenario.endpointId && endpointById(scenario.endpointId)?.modelId === scenario.modelId) {
    params.set("endpoint", scenario.endpointId);
  }
  params.set("mode", scenario.mode);
  params.set("tokens", stableNumber(clampPlanningTokens(scenario.totalTokens)));
  params.set(
    "budget",
    stableNumber(Math.min(MAX_PLANNING_BUDGET_USD, Math.max(0, finite(scenario.budgetUsd)))),
  );
  ...
  params.set(
    "requests",
    stableNumber(
      Math.min(MAX_REQUESTS_PER_MONTH, Math.max(0, Math.round(finite(scenario.requestsPerMonth)))),
    ),
  );
  if (scenario.pinnedModelId && MODEL_CATALOG.some((model) => model.id === scenario.pinnedModelId)) {
    params.set("pin", scenario.pinnedModelId);
  }
