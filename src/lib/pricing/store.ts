...
import { inferenceEndpoints, pricingOverrides, pricingRates } from "@/db/pricing-schema";
...
export async function effectivePublishedPricing(modelId?: string | null) {
  if (!isDatabaseConfigured()) {
    return {
      ...
      snapshot: null,
      data: INFERENCE_ENDPOINTS.filter((endpoint) => !modelId || endpoint.modelId === modelId).map((endpoint) => ({
        ...endpoint,
  ...
  const now = new Date();
  const overrides = await db.select().from(pricingOverrides)
    .where(or(isNull(pricingOverrides.expiresAt), gt(pricingOverrides.expiresAt, now)));
