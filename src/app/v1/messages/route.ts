...
export async function POST(request: Request) {
  const normalized = normalizeCompatibilityRequest(request, await request.json().catch(() => null), { provider: "anthropic", surface: "messages" });
  if (!normalized.ok) return Response.json(normalized, { status: normalized.status, headers: { "Cache-Control": "no-store" } });
  return governedGatewayPost(asInternalGatewayRequest(request, normalized.body), { params: Promise.resolve({ provider: "anthropic" }) });
}
