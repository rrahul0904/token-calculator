...
export default async function SettingsPage() {
  ...
  return <>
    ...
    <div className="app-stack">
      ...
      <DataControlsSettings canManage={canManage} />

      <section className="app-panel"><div className="app-panel__header"><div><h2>Privacy posture</h2><p>Content retention is independent from usage-metadata retention.</p></div></div><div className="app-panel__body"><div className="config-list"><div className="config-row"><span>Legacy telemetry default</span><code>{organization?.retentionDays ?? 90} days</code></div><div className="config-row"><span>Prompt/code persistence</span><code>{organization?.contentRetentionEnabled ? "legacy flag enabled; explicit data-control mode still governs" : "disabled by default"}</code></div><div className="config-row"><span>Gateway content</span><code>transit only by default</code></div><div className="config-row"><span>Provider credentials</span><code>AES-256-GCM + tenant/provider/credential/version AAD</code></div></div></div></section>
    </div>
  </>;
}
