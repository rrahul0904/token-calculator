...
import { dispatchAlert } from "@/lib/alerts/webhooks";
...
export async function POST(request: Request, context: { params: Promise<{ provider: string }> }) {
  ...
  try {
    ...
    if (!rate.allowed) {
      await dispatchAlert({ eventType: "gateway.quota_exceeded", organizationId: principal.organizationId, resourceType: "api_key", resourceId: principal.apiKeyId, data: { reason: "RATE_LIMIT_EXCEEDED", limit: rate.limit, resetAt: rate.resetAt.toISOString() } });
  ...
  try {
    ...
    if (["BLOCK_NEXT_CALL", "KILL_RUN"].includes(result.policyAction)) {
      await dispatchAlert({ eventType: result.policyAction === "KILL_RUN" ? "run.killed" : "budget.blocked", organizationId: principal.organizationId, resourceType: "run", resourceId: result.runId, data: { action: result.policyAction, provider, projectId: parsed.data.projectId ?? principal.projectId } });
    } else if (result.policyAction === "REQUIRE_APPROVAL") {
      await dispatchAlert({ eventType: "fallback.approval_required", organizationId: principal.organizationId, resourceType: "run", resourceId: result.runId, data: { action: result.policyAction, provider, requestedModel: parsed.data.model, fallbackModel: parsed.data.fallbackModel ?? null } });
    } else if (["WARN", "NOTIFY"].includes(result.policyAction)) {
      await dispatchAlert({ eventType: "budget.warned", organizationId: principal.organizationId, resourceType: "run", resourceId: result.runId, data: { action: result.policyAction, provider } });
    }
    ...
    if (["PROVIDER_CONNECTION_NOT_FOUND", "PROVIDER_CONNECTION_NOT_VERIFIED", "PROVIDER_CREDENTIAL_DECRYPTION_FAILED"].includes(message)) {
      await dispatchAlert({ eventType: "provider.connection_failed", organizationId: principal.organizationId, resourceType: "provider_connection", resourceId: parsed.data.providerConnectionId, data: { provider, reason: message } });
    }
