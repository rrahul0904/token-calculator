...
import { inferenceEndpoints, pricingOverrides } from "@/db/pricing-schema";
...
export async function POST(request: Request) {
  ...
  const db = getDb();
  const endpoint = (await db.select({ id: inferenceEndpoints.id })
    .from(inferenceEndpoints)
    .where(eq(inferenceEndpoints.id, parsed.data.endpointId))
    .limit(1))[0];
