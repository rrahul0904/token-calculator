...
import { scenarioVersions } from "@/db/pricing-schema";
...
export async function GET() {
  ...
  try {
    ...
    const rows = await getDb().select().from(savedScenarios)
      .where(eq(savedScenarios.organizationId, tenant.organizationId))
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const workload = workloadScenarioSchema.safeParse(parsed.data.scenario.workload);
    ...
    const row = await db.transaction(async (tx) => {
      ...
      await tx.insert(scenarioVersions).values({
        ...
        assumptions: workload.success ? jsonRecord(workload.data) : parsed.data.scenario,
