...
function costOf(run: typeof runs.$inferSelect): { costUsd: number | null; costSource: ActualCostSource } {
  ...
  if (run.actualCostUsd !== null) {
    return {
      costUsd: Number(run.actualCostUsd),
      costSource: run.usageSource === "provider_measured" ? "provider_measured" : "agent_measured",
    };
  }
...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const db = getDb();
    const scenario = (await db.select().from(savedScenarios)
      .where(and(eq(savedScenarios.id, id), eq(savedScenarios.organizationId, tenant.organizationId)))
      .limit(1))[0];
    if (!scenario) return reply({ error: "SCENARIO_NOT_FOUND" }, 404);

    const run = (await db.select().from(runs)
      .where(and(eq(runs.id, runId), eq(runs.organizationId, tenant.organizationId)))
      .limit(1))[0];
