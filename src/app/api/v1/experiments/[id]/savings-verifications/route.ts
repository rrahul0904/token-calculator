...
export async function POST(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!verification.passed || !verification.savings) {
      return reply({ error: "VERIFIED_SAVINGS_GATE_FAILED", data: verification }, 409);
    }

    const hash = evidenceHash(experiment, rows);
    const db = getDb();
    const existing = (await db.select().from(verifiedSavings)
      .where(and(
    ...
    if (existing) return reply({ data: existing, created: false });

    const latest = (await db.select({ version: verifiedSavings.version }).from(verifiedSavings)
      .where(and(eq(verifiedSavings.experimentId, id), eq(verifiedSavings.organizationId, tenant.organizationId)))
    ...
    await db.insert(verifiedSavings).values(value);
    const created = (await db.select().from(verifiedSavings).where(eq(verifiedSavings.id, value.id)).limit(1))[0];
    return reply({ data: created, created: true }, 201);
  } catch (error) {
    const message = error instanceof Error ? error.message : "AUTHORIZATION_FAILED";
    return reply({ error: message }, message.includes("unique") ? 409 : 403);
  }
}
