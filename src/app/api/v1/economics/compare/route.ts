...
export async function POST(request: Request) {
  ...
  if (!rate.allowed) {
    return Response.json(
      { error: "RATE_LIMITED" },
      { status: 429, headers: { ...rate.headers, "Cache-Control": "no-store" } },
    );
  }
  ...
  if (!parsed.success) {
    return Response.json(
      ...
      { status: 400, headers: { ...rate.headers, "Cache-Control": "no-store" } },
    );
  }

  const baselineId = parsed.data.baselineModelId
    ?? parsed.data.scenario.pinnedModelId
    ?? parsed.data.scenario.modelId;
  const baseline = modelForWorkload(baselineId);
  if (!baseline) {
    return Response.json(
      { error: "BASELINE_MODEL_NOT_FOUND" },
      { status: 404, headers: { ...rate.headers, "Cache-Control": "no-store" } },
    );
  }

  const wanted = parsed.data.candidateModelIds
    ? new Set(parsed.data.candidateModelIds)
    : null;
  const candidates = MODEL_CATALOG.filter(
    (model) => model.status !== "legacy" && (!wanted || wanted.has(model.id)),
  );

  return Response.json(
    {
      data: candidates.map((candidate) =>
        compareWithPinned(baseline, candidate, parsed.data.scenario),
      ),
      qualityGuarantee: false,
      warning:
        "Economic alternatives are not capability-equivalence claims. Quality requires separate evidence.",
    },
